...
Push policy (severity is the 4-tier enum; see classification.py):
...
# `critical` is designed to upgrade to Apple's dedicated `critical` interruption
...
CRITICAL_INTERRUPTION_LEVEL = "time-sensitive"  # -> "critical" once entitled
...
_POLICY: dict[str, PushRule] = {
    "critical": PushRule(True, CRITICAL_INTERRUPTION_LEVEL, DISTINCT_SOUND),
    "urgent": PushRule(True, "time-sensitive", DEFAULT_SOUND),
    "routine": _NO_PUSH,
    "fyi": _NO_PUSH,
}
