...
def test_build_message_critical_distinct_sound():
    call = {
        "id": 7,
        "severity": "critical",
        "patient_name": "Jane D.",
    ...
    assert aps["thread-id"] == "severity-critical"
    assert aps["badge"] == 4
    assert msg["call_id"] == 7


def test_build_message_urgent_default_sound_and_unknown_patient():
    call = {"id": 1, "severity": "urgent", "patient_name": "", "room": "", "summary": "x"}
...
def test_build_message_truncates_summary_to_150():
    call = {"id": 1, "severity": "critical", "patient_name": "A", "room": "", "summary": "y" * 300}
...
def test_sends_for_alerting_severities(monkeypatch, db):
    ...
    _register(db, "tok-1")
    push.dispatch_push_for_call(
        db, {"id": 1, "severity": "critical", "patient_name": "A", "room": "ICU", "summary": "s"}
    )
...
def test_bad_device_token_is_removed(monkeypatch, db):
    ...
    push.dispatch_push_for_call(
        db, {"id": 1, "severity": "critical", "patient_name": "A", "room": "", "summary": "s"}
    )
